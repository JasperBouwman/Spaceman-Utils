...
public class ColorTheme implements ConfigurationSerializable {
    /*
     * When using this util (and pre-written command) allows players to customize their view of your plugin with custom color themes.
     *
     * This util uses the following utils:
     * - file handler
     * - command handler (only when using ColorThemeCommand)
     *
     * Example usage: sendInfoTheme(player, "%s is set to the value &s", "Some object", "some value");
     *
     * in your onEnable:
     * ConfigurationSerialization.registerClass(ColorTheme.class, "ColorTheme");
     * //file registration (GettingFiles.loadFiles();)
     * ColorTheme.loadThemes(file);
     *
     * in your onDisable:
     * ColorTheme.saveThemes(GettingFiles.getFile("configFile"));
     *
     * */
