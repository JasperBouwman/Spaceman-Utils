...
import com.spaceman.colorFormatter.ColorFormatter;
...
import static com.spaceman.fancyMessage.events.HoverEvent.hoverEvent;
...
public class KeyValueHelper {
    ...
    public static HashMap<String, Object> constructObject(String stringObj, List<? extends Key> keys) throws KeyValueError {
        ...
        for (String keyValue : stringObj.split(",")) {
            if (keyValue.matches(".+=.+")) {
                ...
                if (newKey != null) {
                    ...
                    } else if (value == null && !newKey.isAcceptNullValue()) {
                        Message message = new Message();
                        message.addText(textComponent(split[1], ColorFormatter.varErrorColor));
                        message.addText(textComponent(newKey.getErrorMessage(), ColorFormatter.errorColor));
                        throw new KeyValueError(message);
                    } else {
                        keysMap.put(newKey.getKey(), value);
                        neededList.remove(split[0].toLowerCase());
                    }
                } else {
                    Message message = new Message();
                    message.addText(textComponent("Key ", ColorFormatter.errorColor));
                    message.addText(textComponent(keyValue.split("=")[0], ColorFormatter.varErrorColor));
                    message.addText(textComponent(" is not accepted, please use the format: ", ColorFormatter.errorColor));
                    message.addMessage(getFormat(keys));
                    throw new KeyValueError(message);
                }
            } else {
                Message message = new Message();
                message.addText(textComponent("Error in ", ColorFormatter.errorColor));
                message.addText(textComponent(keyValue, ColorFormatter.varErrorColor));
                message.addText(textComponent(", please use the format: ", ColorFormatter.errorColor));
                message.addMessage(getFormat(keys));
                throw new KeyValueError(message);
            }
        }
        ...
        if (!neededList.isEmpty()) {
            Message message = new Message();
            message.addText(textComponent("Not all needed keys are used, please use the format: ", ColorFormatter.errorColor));
            message.addMessage(getFormat(keys));
            throw new KeyValueError(message);
        }
    ...
    private static Message getFormat(List<? extends Key> keys) {
        ...
        for (Key key : keys) {
            if (key.getKey() != null && !key.getKey().equals("")) {
                if (key.isOptional()) {
                    message.addText(textComponent(key.getKey() + "=[value]", ColorFormatter.varErrorColor, hoverEvent(textComponent("This Key is optional", ColorFormatter.errorColor))));
                } else {
                    message.addText(textComponent(key.getKey() + "=<value>", ColorFormatter.varErrorColor));
                }
            }
            message.addText(textComponent(",", ColorFormatter.varErrorColor));
        }
        message.removeLast();
        return message;
    }
